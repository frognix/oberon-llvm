...
ParserPtr<T> change(ParserPtr<P> parser, T ch) {
    return extension(parser, [ch](const auto&) {
        return ch;
    });
}
        ...
        if (auto bres = m_begin->parse(stream); bres) {
            while (true) {
                if (auto eres = m_end->parse(stream); eres) {
                    ...
                    if (eres.err_ref().is_undroppable()) return eres.get_err();
                    result.push_back(pres.get_ok());
                } else {
                    point.error(pres.err_ref());
                    if (pres.err_ref().is_undroppable()) return pres.get_err();
...
ParserPtr<Module> get_parser() {
    ...
    term = node_either<Expression>(construct<Term>(syntax_sequence(factor,
    ...
    simpleExpression = node_either<Expression>(construct<Term>(syntax_sequence(maybe(either({symbol('+'), symbol('-')})),
                                                                                                        term,
    ...
    expression = node_either<Expression>(construct<Term>(syntax_sequence(simpleExpression,
    ...
    statement = node_either<Statement>(assignment, procCall, ifStatement, caseStatement, whileStatement, repeatStatement, forStatement);
    ...
    procedureDecl = node_either<CodeBlock>(parse_index<0>::tuple_select(except(syntax_sequence(procDeclBase, ident), "same ident", [](const auto& pair){
    ...
    auto module = parse_index<0>::tuple_select(except(syntax_sequence(moduleBase, ident, symbol('.')), "same module name", [](const auto& pair){
